...
/** Pastel chip fill for the tag wheel — blends tag color toward cream paper. */
...
/** Whisper-soft fill for tags on item cards — barely tinted paper. */
export function tagItemChipFill(color: string): string {
  const rgb = hexToRgb(color);
  if (!rgb) return "rgba(248, 250, 252, 0.9)";
  const mix = 0.84;
  const cream = { r: 255, g: 254, b: 251 };
  return rgbToHex(
    rgb.r + (cream.r - rgb.r) * mix,
...
/** Label color on pastel wheel chips — slightly darkened tag hue. */
...
/** Muted label on item cards — readable but unobtrusive. */
...
/** Notebook tag pill — soft gradient fill, dark label, logo brush accent. */
