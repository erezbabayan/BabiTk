...
function useTaskListsOffline(_userId: Id<"users"> | undefined) {
  ...
  const noopNum = useCallback(async () => 0, []);
  return {
    enabled: false,
    lists: [] as TaskListRecord[],
    loading: false,
    createFromTags: async (
      _filterTags: string[],
      _name: string,
      _boardTasks: MindtaskerItem[] = [],
    ) => undefined,
    renameList: noop,
    refreshListTags: noop,
    archiveList: noop,
    restoreList: noop,
    deleteList: noop,
    refreshListItems: noopNum,
    setListReminder: noop,
    clearListReminder: noop,
  };
}

function useTaskListsConvex(userId: Id<"users"> | undefined) {
  ...
  const setListReminder = useCallback(
    ...
    [ensureSoonReminder, updateMutation, userId],
  );
...
export const useTaskLists = OFFLINE ? useTaskListsOffline : useTaskListsConvex;
