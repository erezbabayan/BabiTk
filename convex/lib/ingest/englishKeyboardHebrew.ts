 ...
 *
 * Conservative: only rewrites tokens that look like mistyped Hebrew,
 * leaving real English words, URLs, and already-Hebrew text alone.
 */

/** Physical QWERTY key → Hebrew letter on the standard Israeli keyboard. */
...
function shouldKeepEnglishToken(token: string): boolean {
  ...
  // Keep mixed alphanumerics (codes, versions).
...
/**
 * If `text` (or parts of it) was typed with English layout while intending
 * Hebrew, return the corrected string; otherwise return the original.
 */
export function correctEnglishKeyboardHebrew(text: string): string {
  ...
  // Already mostly Hebrew — don't rewrite residual Latin.
  if (hebrew >= 3 && hebrew >= latin) return text;

  // Split on whitespace; keep separators so spacing/newlines survive.
  ...
  // Avoid rewriting a mostly-English sentence that happened to convert one token.
