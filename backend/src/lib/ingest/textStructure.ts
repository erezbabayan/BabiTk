 ...
 * Keep in sync with convex/lib/ingest/textStructure.ts
 */
...
export function preserveInboundLineStructure(text: string): string {
  return text
    ...
    .split("\n")
    .map((line) => {
      // Keep leading indentation (list/nesting cues) and collapse only inner runs.
      const indent = /^[ \t\u00a0]*/.exec(line)?.[0] ?? "";
      return (
        indent + line.slice(indent.length).replace(/[ \t\u00a0]+/g, " ")
      ).trimEnd();
    })
    .join("\n")
    .replace(/\n{3,}/g, "\n\n")
