...
async function readLocalLists(userId: string): Promise<TaskListRecordShape[]> {
  try {
...
async function writeLocalLists(
  userId: string,
  lists: TaskListRecordShape[],
): Promise<void> {
...
export async function createTaskListsFromTags(
  ...
  await withTableFallback(
    ...
    async () => {
      await writeLocalLists(userId, [...existing.map(toTaskListRecordShape), ...created]);
    },
  );
...
async function patchTaskList(
  ...
  await withTableFallback(
    ...
    async () => {
      const lists = await readLocalLists(userId);
      ...
      await writeLocalLists(userId, next);
    },
  );
}
