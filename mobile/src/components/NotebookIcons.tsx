...
import type { SwipeActionIconName } from "../lib/swipe-action-style";
...
interface NotebookIconProps {
  ...
  /** Override stroke/fill color (used by swipe strips). */
  color?: string;
  /** Filled star for priority markers. */
  filled?: boolean;
}

type IconRender = (stroke: string) => React.ReactNode;

const ICON_PATHS: Record<NotebookIconName, IconRender> = {
  calendar: (stroke) => (
    <>
  ...
  swap: (stroke) => (
    <>
      <Path
        d="M7 8.5h11M7 8.5l2.5-2.5M7 8.5l2.5 2.5"
        stroke={stroke}
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M17 15.5H6M17 15.5l-2.5-2.5M17 15.5l-2.5 2.5"
        stroke={stroke}
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
  ...
  sort: (stroke) => (
    <>
      <Path
        d="M8 6v12M8 18l-2.5-2.5M8 18l2.5-2.5"
        stroke={stroke}
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <Path
        d="M16 18V6M16 6l2.5 2.5M16 6l-2.5 2.5"
        stroke={stroke}
        strokeWidth={1.5}
        strokeLinecap="round"
        strokeLinejoin="round"
  ...
  keyboard: (stroke) => (
    <>
      ...
      <Path
        d="M7 11h.01M11 11h.01M15 11h.01M7 14h10"
        stroke={stroke}
        strokeWidth={1.75}
        strokeLinecap="round"
      />
  ...
  grip: (stroke) => (
    <>
      ...
      <Circle cx="15" cy="8" r="1" fill={stroke} />
...
export function NotebookIcon({
  ...
  const stroke = color ?? TONE_STROKE[tone];
...
/** Swipe strip helper — same paths as web NotebookIcon. */
export function NotebookSwipeIcon({
  name,
  size = 22,
  color = "#64748b",
}: {
  name: SwipeActionIconName;
  size?: number;
  color?: string;
}) {
  return <NotebookIcon name={name} size={size} color={color} />;
}
